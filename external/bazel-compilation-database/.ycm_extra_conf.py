...
def standardize_file_target(file_target):
    ...
    if not query_result:
        sys.exit(
            "Empty query response for {}. It is probably not handled by bazel".format(
                file_target
            )
        )
...
def cfamily_settings(filename):
        ...
        [
            (
                'kind("{cc_rules}", rdeps(siblings({f}), {f}, 1))'.format(
                    f=file_target, cc_rules=cc_rules
                )
            ),
            "--keep_going",
        ]
    ...
    proc = subprocess.Popen(
        bazel_aspects, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    out, err = proc.communicate()
    if proc.returncode != 0:
        errors = [
            e for e in out.splitlines() + err.splitlines() if e.startswith("ERROR:")
        ]
        if errors:
