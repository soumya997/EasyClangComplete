...
class TestCompilationDb(object):
    ...
    def test_get_all_flags(self):
        ...
        path_to_db = path.join(
            path.dirname(__file__), "compilation_db_files", "command"
        )
    ...
    def test_strip_wrong_arguments(self):
        ...
        path_to_db = path.join(
            path.dirname(__file__), "compilation_db_files", "arguments"
        )
        scope = SearchScope(from_folder=path_to_db)
        if self.lazy_parsing:
            import sublime
            ...
            if sublime.platform() != "windows":
                ...
                self.assertIn(
                    Flag("", "-Dlib_EXPORTS"),
                    db.get_flags(file_path=file_path, search_scope=scope),
                )
                self.assertIn(
                    Flag("", "-fPIC"),
                    db.get_flags(file_path=file_path, search_scope=scope),
    ...
    def test_get_flags_for_path(self):
        ...
        self.assertEqual(path_to_db, db._cache[lib_file_path])
        self.assertEqual(path_to_db, db._cache[main_file_path])

        if self.lazy_parsing:
            self.assertNotIn(CompilationDb.ALL_TAG, db._cache[path_to_db])
        else:
            self.assertIn(expected_main, db._cache[path_to_db][CompilationDb.ALL_TAG])
            self.assertIn(expected_lib[0], db._cache[path_to_db][CompilationDb.ALL_TAG])
            self.assertIn(expected_lib[1], db._cache[path_to_db][CompilationDb.ALL_TAG])
    ...
    def test_persistence(self):
        ...
        scope = SearchScope(from_folder=path_to_db)
        self.assertIn(Flag("", "-Dlib_EXPORTS"), db.get_flags(lib_file_path, scope))
        self.assertIn(Flag("", "-fPIC"), db.get_flags(lib_file_path, scope))
        self.assertIn(expected_main, db.get_flags(main_file_path, scope))
        # check persistence
        self.assertGreater(len(db._cache), 2)
        self.assertEqual(
            path.join(path_to_db, "compile_commands.json"), db._cache[main_file_path]
        )
        self.assertEqual(
            path.join(path_to_db, "compile_commands.json"), db._cache[lib_file_path]
        )
    ...
    def test_relative_directory(self):
        ...
        path_to_db = path.realpath(
            path.join(path.dirname(__file__), "compilation_db_files", "directory")
        )
        scope = SearchScope(from_folder=path_to_db)
        if self.lazy_parsing:
            import sublime
            ...
            if sublime.platform() != "windows":
                ...
                self.assertEqual(
                    expected, db.get_flags(file_path=file_path, search_scope=scope)
                )
            # Check that we don't get the 'all' entry.
            self.assertIsNone(db.get_flags(search_scope=scope))
        else:
    ...
    def test_get_c_flags(self):
        ...
        # also try to test a header
        path_to_db = path.join(
            path.dirname(__file__), "compilation_db_files", "command_c"
        )
        scope = SearchScope(from_folder=path_to_db)
    ...
    def test_get_c_flags_ccache(self):
        ...
        # also try to test a header
        path_to_db = path.join(
            path.dirname(__file__), "compilation_db_files", "command_c_ccache"
        )
        scope = SearchScope(from_folder=path_to_db)
        flags = db.get_flags(main_file_path, scope)
