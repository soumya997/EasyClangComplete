...
class TestCmakeFile(object):
    ...
    def test_cmake_generate(self):
        ...
        test_file_path = path.join(path.dirname(__file__), "cmake_tests", "test_a.cpp")
        ...
        expected_cmake_file = path.join(path_to_cmake_proj, CMakeFile._FILE_NAME)
    ...
    def test_cmake_with_existing_header(self):
        """Test that cmake can generate flags."""
        test_file_path = path.join(path.dirname(__file__), "cmake_tests", "lib", "a.h")
        ...
        flags = cmake_file.get_flags(test_file_path)
        db = CompilationDb(
            ["-I", "-isystem"], header_to_source_map=[], lazy_flag_parsing=False
        )
    ...
    def test_cmake_get_deps(self):
        ...
        self.assertEqual(res[1], "/usr/share/cmake-3.5/Modules/CMakeCCompiler.cmake.in")
        self.assertEqual(
            res[-1],
            path.join(parent_folder, "lib/CMakeFiles/liba.dir/DependInfo.cmake"),
        )
