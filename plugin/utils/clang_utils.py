...
class ClangUtils:
    ...
    SORTED_OSX_VERSION_KEYS = [
        str(k) for k in sorted([float(key) for key in OSX_CLANG_VERSION_DICT.keys()])
    ]

    CINDEX_DICT = {
        "3.2": PKG_NAME + ".plugin.clang.cindex50",
        "3.3": PKG_NAME + ".plugin.clang.cindex50",
        "3.4": PKG_NAME + ".plugin.clang.cindex50",
        "3.5": PKG_NAME + ".plugin.clang.cindex50",
        "3.6": PKG_NAME + ".plugin.clang.cindex50",
        "3.7": PKG_NAME + ".plugin.clang.cindex50",
        "3.8": PKG_NAME + ".plugin.clang.cindex50",
        "3.9": PKG_NAME + ".plugin.clang.cindex50",
        "4.0": PKG_NAME + ".plugin.clang.cindex50",
        "5.0": PKG_NAME + ".plugin.clang.cindex50",
    }

    SORTED_CINDEX_KEYS = sorted(CINDEX_DICT)
    ...
    def prepare_search_libclang_cmd(clang_binary, lib_file_name):
        ...
        if platform.system() == "Darwin":
            ...
            get_library_path_cmd = [
                clang_binary,
                "-print-file-name={}".format(lib_file_name),
            ]
    ...
    def find_libclang(clang_binary, libclang_path, version_str):
        ...
        """
        log.debug("Platform: %s, %s", platform.system(), platform.architecture())
        ...
        if path.exists(libclang_path):
            ...
            if user_libclang_dir and libclang_file:
                ...
                return user_libclang_dir, path.join(user_libclang_dir, libclang_file)
        ...
        for libclang_filename in possible_filenames:
            ...
            if user_libclang_dir:
                log.debug("Searching in user provided folder: '%s'", user_libclang_dir)
                user_hinted_file = path.join(user_libclang_dir, libclang_filename)
            ...
            (
                ...
                stderr,
                startupinfo,
            ) = ClangUtils.prepare_search_libclang_cmd(clang_binary, libclang_filename)
            output = (
            ...
            if output:
                ...
                if path.isdir(libclang_dir):
                    full_libclang_path = path.join(libclang_dir, libclang_filename)
                    log.debug("Checking path: %s", full_libclang_path)
                    if path.exists(full_libclang_path):
                        log.info(
                            "Found libclang library file: '%s'", full_libclang_path
                        )
    ...
    def get_clang_version_str(cls, clang_binary):
        ...
        log.info("Getting version from command: `%s`", " ".join(check_version_cmd))
    ...
    def _get_apple_clang_version_str(cls, output_text):
        ...
        if match:
            ...
            try:
                ...
                raise e
            log.warning(
                "OSX version %s reported. Reducing it to %s.", osx_ver, version_str
            )
